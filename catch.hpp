...
namespace Catch {
    ...
    public: // substrings and searches
        ...
        // Returns the current start _pointer.
        // Note that the _pointer can change when if the StringRef is a substring
...
namespace Catch {
    ...
    struct CumulativeReporterBase : IStreamingReporter {
        ...
        bool assertionEnded(AssertionStats const& assertionStats) override {
            ...
            // AssertionResult holds a _pointer to a temporary DecomposedExpression,
...
namespace TestCaseTracking {
    ...
    public:
        TrackerBase( NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* _parent );
    ...
    public:
        SectionTracker( NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* _parent );
    ...
    class IndexTracker : public TrackerBase {
        ...
        int m_index = -1;
    public:
        IndexTracker( NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* _parent, int size );
...
namespace TestCaseTracking {
    ...
    TrackerBase::TrackerBase( NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* _parent )
    ...
    SectionTracker::SectionTracker( NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* _parent )
    :   TrackerBase( nameAndLocation, ctx, _parent )
    {
        if( _parent ) {
            while( !_parent->isSectionTracker() )
                _parent = &_parent->_parent();

            SectionTracker& parentSection = static_cast<SectionTracker&>( *_parent );
    ...
    IndexTracker::IndexTracker( NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* _parent, int size )
    :   TrackerBase( nameAndLocation, ctx, _parent ),
        m_size( size )
    {}
